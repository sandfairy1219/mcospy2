...
// });
function getFilteredEntityList(exceptMark) {
    if (!epos)
        return [];
    if (epos.isNull())
        return [];
...
function saveWPData(pointer) {
    ...
    if (zr1 !== 0 || zr2 !== 0 || nick === "" || n.startsWith("-"))
        return;
    ...
    if (wpdata[n]) {
        ...
        if (Number(char) > 0)
            wpdata[n].chars[char] = { exp, totalkill, totaldeath, totalassist, date: Date.now() };
    }
    else {
        wpdata[n] = {
...
let shooting = false;
function makeNFunc(symbol, retType, argTypes) {
    try {
...
function attachNFunc(symbol, callbacksOrProbe) {
...
function init() {
    try {
        ...
        setDia = (amount) => makeNFunc('_ZN16SystemPacketSend13CheatSetMoneyEii', 'void', ['int', 'int'])(amount, 0);
        setGold = (amount) => makeNFunc('_ZN16SystemPacketSend12CheatSetGoldEii', 'void', ['int', 'int'])(amount, 0);
        setXp = (amount) => makeNFunc('_ZN16SystemPacketSend21CheatSetGradeAndPointEhj', 'void', ['uchar', 'uint'])(1, amount);
        ...
        createClan = (name = genRandom(), desc = " ", mark = 0, flag = 151) => {
        ...
        function api(message) {
            try {
                ...
                else if (name === 'match-draw') {
                    endgame(ptr(3));
                }
                else if (name === 'match-milk') {
                    endgame(ptr(0));
                }
                else if (name === 'match-choco') {
                    endgame(ptr(1));
                }
                else if (name === 'receive-dia') {
                    setDia(+args[0] || 0);
                }
                else if (name === 'receive-gold') {
                    setGold(+args[0] || 0);
                }
                else if (name === 'receive-xp') {
                    setXp(+args[0] || 0);
                }
                else if (name === 'receive-clan-xp') {
                    setClanXp(+args[0] || 0);
                }
                else if (name === 'receive-sl-coin') {
                    setSLCoin(+args[0] || 0);
                }
                else if (name === 'receive-sl-point') {
                    setSLPoint(+args[0] || 0);
                }
                else if (name === 'unlock-sl-medal') {
                    unlockSLMedal();
                }
                else if (name === 'unlock-all-item') {
                    unlockChar(+args[0] || 0);
                }
                else if (name === 'unlock-all-char') {
                    unlockAllChar();
                }
                ...
                else if (name === 'purchase-pass') {
                    purchaseP(+args[0] || 0, +args[1] || 0);
                }
                else if (name === 'server-exploit') {
                    exploitServer();
                }
                else if (name === 'create-clan') {
                    createClan(args[0] || genRandom());
                }
                else if (name === 'break-clan') {
                    breakClan();
                }
                else if (name === 'equip-item') {
                    equip(+args[0] || 1, +args[1] || 0, +args[2] || 0);
                }
                else if (name === 'ctm-default-milk') {
                    ctmDefaultMilk();
                }
                else if (name === 'ctm-default-choco') {
                    ctmDefaultChoco();
                }
                else if (name === 'ctm-desert-milk') {
                    ctmDesertMilk();
                }
                else if (name === 'ctm-desert-choco') {
                    ctmDesertChoco();
                }
                else if (name === 'ctm-castle-milk') {
                    ctmCastleMilk();
                }
                else if (name === 'ctm-castle-choco') {
                    ctmCastleChoco();
                }
                else if (name === 'ctm-mountain-milk') {
                    ctmMountainMilk();
                }
                else if (name === 'ctm-mountain-choco') {
                    ctmMountainChoco();
                }
                else if (name === 'deathmatch-start') {
                    deathmatchStart();
                }
...
function loop() {
    ...
    try {
        if (epos && !epos.isNull()) {
            ...
            if (cheats['esp']) {
                if (cambase && !cambase.isNull() && entityList.size > 0) {
                    ...
                    const data = entities.map(entity => {
                        ...
                        let nickname = '';
                        try {
                            nickname = entity.add(eposOffset['nickname']).readUtf8String() || '';
                        }
                        catch (_) {
                            nickname = entity.add(eposOffset['nickname']).readCString() || '';
                        }
            ...
            if (cheats['no-timer']) {
                const reloadTimer = config['no-timer-reload'] || false;
                const grenadeTimer = config['no-timer-grenade'] || false;
                const respawnTimer = config['no-timer-respawn'] || false;
                if (reloadTimer &&
                ...
                if (respawnTimer && state === 16) {
                    ...
                    if (dc != 1 && dc != 0)
                        epos.add(eposOffset['dc']).writeFloat(1);
                }
            }
            if (cheats['move-speed']) {
                if ((!keybinds['move-speed'] || keymap[keybinds['move-speed']])) {
                    const half = Math.sin(45 / 180 * Math.PI);
                    const val = (+config['move-speed-value']) || 3;
                    if (keymap["W"] && keymap["A"])
                        epos.add(eposOffset['dx']).writeFloat(half * val), epos.add(eposOffset['dz']).writeFloat(half * val);
                        ...
                        epos.add(eposOffset['dx']).writeFloat(val);
                    else if (keymap["S"])
                        epos.add(eposOffset['dx']).writeFloat(-val);
                    else if (keymap["A"])
                        epos.add(eposOffset['dz']).writeFloat(val);
                    else if (keymap["D"])
                        epos.add(eposOffset['dz']).writeFloat(-val);
                    else {
            ...
            if (cheats['fly']) {
                ...
                epos.add(eposOffset['fall']).writeS8(0);
                const y = +epos.add(eposOffset['y']).readFloat();
                if (keybinds['fly-up'] && keymap[keybinds['fly-up']])
                    epos.add(eposOffset['y']).writeFloat(y + (+config['fly-speed'] || 1));
                if (keybinds['fly-down'] && keymap[keybinds['fly-down']])
                    epos.add(eposOffset['y']).writeFloat(y - (+config['fly-speed'] || 1));
            }
            if (cheats['freecam']) {
            ...
            if (cheats['upskill'] && (!keybinds['upskill'] || keymap[keybinds['upskill']])) {
                ...
                switch (char) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        epos.add(eposOffset['sc']).writeFloat(9999);
                        break;
                    case 5:
                        if (isSkill(epos)) {
                            if (onlyonce) {
...
function scanEpos() {
    ...
    cas = Process.enumerateRanges('rw-').filter(range => !range.file &&
...
function scanEntityList(_eposPointer) {
    if (!_eposPointer)
        return new Set();
    if (_eposPointer.isNull())
        return new Set();
    if (_eposPointer.add(eposOffset['pointer']).isNull())
        return new Set();
    let _entityList = new Set();
    ...
    try {
        log("Entity Found:", _entityList.size, '\n', [..._entityList].map(entity => { let n = ''; try {
            n = entity.add(eposOffset['nickname']).readUtf8String() || '';
        }
        catch (_) {
            n = entity.add(eposOffset['nickname']).readCString() || '';
        } return `${entity.toString()} [${entity.add(eposOffset['number']).readS32()}] ${n}`; }).join('\n'));
    }
    catch (e) {
...
function aimbot(eposPointer, delta) {
    if (!cambase)
        return;
    if (!eposPointer)
        return;
    ...
    if (eposPointer.isNull())
        return;
    if (isDead(eposPointer))
        return;
    if (config['aimbot-main-weapon-only'] && eposPointer.add(eposOffset["weapon"]).readS16() == 1)
        return;
    ...
    const targets = getFilteredEntityList(ignoreExcept)
        ...
        if (config["aimbot-acceleration"]) {
            ...
            if (!isGround(entity))
                dy -= entity.add(eposOffset["dy"]).readFloat();
            dz += entity.add(eposOffset["dz"]).readFloat();
            dx -= eposPointer.add(eposOffset["dx"]).readFloat();
            if (!isGround(eposPointer))
                dy += eposPointer.add(eposOffset["dy"]).readFloat();
    ...
    }).filter((d) => d[4] <= rad).sort((a, b) => a[4] - b[4]);
...
function aimassist(eposPointer, delta) {
    if (!cambase)
        return;
    if (!eposPointer)
        return;
...
function calcESP(vec3, cam3, yaw, pitch, fov) {
    const camX = cam3.x;
    const camY = cam3.y;
    const camZ = cam3.z;
    ...
    let rotatedX = dx * Math.cos(yaw + Math.PI) - dz * Math.sin(yaw + Math.PI);
    let rotatedZ = dz * Math.cos(yaw + Math.PI) + dx * Math.sin(yaw + Math.PI);
    let rotatedY = -dy * Math.cos(-pitch) + rotatedZ * Math.sin(-pitch);
...
function blackhole(eposPointer) {
    if (!cambase)
        return;
    if (!eposPointer)
        return;
...
function isShooting(eposPointer) {
    if (!eposPointer)
        return false;
    if (eposPointer.isNull())
        return false;
...
function isWalking(eposPointer) {
    if (!eposPointer)
        return false;
    if (eposPointer.isNull())
        return false;
...
function isTeam(my, eposPointer) {
    if (!eposPointer)
        return false;
    if (eposPointer.isNull())
        return false;
...
function executeMacro(events) {
    ...
    events.forEach(event => {
        if (event.base === "epos") {
            const offset = eposOffset[event.offset];
            ...
            const offset = event.base === "xa" ? xaOffset[event.offset] : anOffset[event.offset];
            const value = event.value;
            const type = event.type;
            if (type === "byte") {
                base.add(offset).writeS8(value);
...
async function teleport(eposPointer, x, y, z) {
    if (!eposPointer)
        return;
    if (eposPointer.isNull())
        return;
...
rpc.exports = {
    readArrayBytes: function (addr, size, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
        const res = pointer.readByteArray(size);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r--');
        return res;
    },
    readByte: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    readInt16: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    readInt32: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    readFloat: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    readDouble: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    readPointer: function (addr, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    ...
    writeArrayBytes: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
        pointer.writeByteArray(data);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'r-x');
    },
    writeByte: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
    ...
    writeInt16: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
    ...
    writeInt32: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
    ...
    writeFloat: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
    ...
    writeDouble: function (addr, data, ignoreProtection = false) {
        const pointer = ptr(addr);
        if (ignoreProtection)
            Memory.protect(pointer, Process.pageSize, 'rwx');
    ...
    writePointer: function (addr, data, ignoreProtection = false) {
        if (ignoreProtection)
            Memory.protect(ptr(addr), Process.pageSize, 'rwx');
        ptr(addr).writePointer(ptr(data));
        if (ignoreProtection)
            Memory.protect(ptr(addr), Process.pageSize, 'r-x');
    },
...
function getChainedPointer(_bs, iter) {
    let pt = _bs;
...
function qwordToHex(qword) {
    const x = qword.toString(16);
    return x.match(/.{2}/g).reverse().join(' ');
}
function withWritable(_ptr, fn) {
    let base = _ptr;
    let size = Process.pageSize;
    let originalProt = 'r-x';
    try {
        const range = Process.getRangeByAddress(_ptr);
        base = range.base;
        size = range.size;
        originalProt = range.protection;
    }
    catch (_) {
...
function encodeExpBoost(multiplier) {
...
function applyNoSpread(enabled) {
...
function applyNoSpreadSub(configKey) {
    if (!cheats['no-spread'])
        return;
...
function demangle(name) {
    ...
    try {
            ...
            switch (process.ch) {
                ...
                case 'S':
                    ...
                    switch (process.ch) {
                        ...
                        case 'a':
                            typeInfo.typeStr = "std::allocator";
                            break;
                        case 'b':
                            typeInfo.typeStr = "std::basic_string";
                            break;
                        case 's':
                            typeInfo.typeStr = "std::basic_string<char, std::char_traits<char>, std::allocator<char>>";
                            break;
                        case 'i':
                            typeInfo.typeStr = "std::basic_istream<char, std::char_traits<char>>";
                            break;
                        case 'o':
                            typeInfo.typeStr = "std::basic_ostream<char, std::char_traits<char>>";
                            break;
                        case 'd':
                            typeInfo.typeStr = "std::basic_iostream<char, std::char_traits<char>>";
                            break;
                        default:
                ...
                case 'j':
                    ...
                    break;
                case 'l':
                    typeInfo.typeStr = "long int";
                    break;
                case 'm':
                    typeInfo.typeStr = "unsigned long int";
                    break;
                case 'x':
                    typeInfo.typeStr = "long long int";
                    break;
                case 'y':
                    typeInfo.typeStr = "unsigned long long int";
                    break;
                case 'n':
        ...
        const typelist = types.map((t) => {
            let typestr = "";
            if (t.isConst)
                typestr = typestr.concat("const ");
            if (t.isVolatile)
                typestr = typestr.concat("volatile ");
            ...
            if (!t.templateStart) {
                ...
                if (t.isRValueRef)
                    typestr = typestr.concat("&&");
                for (let i = 0; i < t.numPtr; i++)
                    typestr = typestr.concat("*");
                if (t.isRestrict)
                    typestr = typestr.concat(" __restrict");
            }
            if (t.templateType) {
                if (t.templateType.isRef)
                    typestr = typestr.concat("&");
                if (t.templateType.isRValueRef)
                    typestr = typestr.concat("&&");
                for (let i = 0; i < t.templateType.numPtr; i++)
                    typestr = typestr.concat("*");
            }
            return typestr;
        });
...
function cmdHookF(str) {
    const demangled = demangle(str);
    try {
        Interceptor.attach(_libMyGame.findExportByName(str), {
            onEnter: (args) => {
...
function cmdReadF(str, type) {
    try {
        const pt = ptr(str);
        if (!pt)
            return log("null pointer");
        if (pt.isNull())
            return log("null pointer");
...
function cmdWriteF(str, type, value) {
    try {
        const pt = ptr(str);
        if (!pt)
            return log("null pointer");
